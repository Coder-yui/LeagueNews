"""Media evidence preparation and approved-extraction helpers for V3."""
...
def is_patch_preview(raw_item: RawItem) -> bool:
    ...
    preview_language = any(marker in text for marker in ("preview", "micropatch", "hotfix"))
...
async def extract_patch_preview(
    ...
    *,
        ...
        existing_extractions = list(
            db.scalars(
                ...
                .where(
                    ...
                    *(
                        (MediaExtraction.processing_run_id == processing_run_id,)
                        if artifact_scope != "production" and processing_run_id is not None
                        else ()
                    ),
                )
    ...
    if structure:
        structured_data = build_patch_preview(
            title=raw_item.display_title,
            table_data=table.model_dump(),
        ).model_dump(mode="json")
    if artifact_scope == "production":
...
def structure_patch_extraction(
    ...
    *,
    ...
    structured = build_patch_preview(title=title, table_data=table_data)
            ...
            entries.append(
                {
                    ...
                    "changes": [str(value).strip() for value in raw_changes if str(value).strip()],
                }
            )
        sections.append(
            {
