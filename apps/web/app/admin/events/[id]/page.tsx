...
export default async function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="admin-page admin-detail-page">
    ...
    <header className="admin-detail-head"><div className="admin-badge-row"><span className="admin-badge">{event.event_kind}</span><span className="admin-badge subtle">{event.aggregation_strategy}</span><span className="admin-badge subtle">{event.lifecycle_status}</span><span className="admin-badge">重要性 {pointScore(event.importance_score)} / 100</span><span className="admin-badge success">{event.credibility_status} · {score(event.credibility_score)}</span></div><h1>{event.title}</h1><code>{event.aggregation_key ?? "无聚合键"}</code><p>{event.summary}</p>{event.latest_development && <div className="admin-latest"><strong>最新进展</strong>{event.latest_development}</div>}</header>
