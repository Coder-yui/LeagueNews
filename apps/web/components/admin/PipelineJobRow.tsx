...
import { canonicalStage } from "./admin-utils";
...
export function PipelineJobRow({ job, onRecovered }: { job: PipelineJob; onRecovered: () => void }) {
  ...
  const retry = async () => { setBusy(true); setError(null); try { await adminApi(`/pipeline/jobs/${job.id}/recover`, { method: "POST", body: JSON.stringify({ restart_from_stage: canonicalStage(job.current_stage), resume_mode: "automatic", reason: "管理台手动重试失败任务" }) }); onRecovered(); } catch (value) { setError(value instanceof Error ? value.message : "重试失败"); } finally { setBusy(false); } };
