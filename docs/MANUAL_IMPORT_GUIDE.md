...
如需把消息纳入事件层，在单条消息全部审核批准后继续：

- 在管理台“事件聚合”的未审核页显式启动该消息的事件判断。
- 在“审核中心 → 事件聚合审核”检查 `not_event`、`create` 或 `update` 决策。
- 批准后，在公开事件列表或对应事件详情中核对事件卡片、时间线、可信度、重要性和
  最新进展。

事件聚合不会在导入后自动运行，也不会修改 `RawItem` 或已经批准的单条消息内容。
