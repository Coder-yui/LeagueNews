...
def test_candidate_recall_window_is_60_days() -> None:
    engine = _engine()
    with Session(engine, expire_on_commit=False) as db:
        ...
        db.flush()
        now = datetime(2026, 8, 12, 8, tzinfo=UTC)
        for name, age_days in (("recent", 59), ("old", 61)):
            db.add(
                Event(
                    title=f"星界事件 {name}",
                    current_summary=f"共同实体 星界 {name}",
                    event_family="gameplay_balance",
                    products=["lol_pc"],
                    canonical_anchors={"entity": "星界"},
        ...
        item = _item(
            db,
            source=source,
            external_id="recall-window-message",
            title="星界相关更新",
            text="星界相关更新内容。",
            entities=[{"type": "activity", "name": "星界"}],
            products=["lol_pc"],
        ...
        assert [candidate["title"] for candidate in candidates] == ["星界事件 recent"]
