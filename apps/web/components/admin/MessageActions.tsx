...
export function MessageActions({ itemId }: { itemId: number }) {
  const [stage, setStage] = useState<PipelineStageName>("classify"); const [busy, setBusy] = useState(false); const [message, setMessage] = useState<string | null>(null); const [events, setEvents] = useState<EventSummary[]>([]); const [eventId, setEventId] = useState<number | null>(null); const [role, setRole] = useState("primary");
  ...
  return <div className="admin-action-panel"><label>从阶段重跑<select value={stage} onChange={(event) => setStage(event.target.value as PipelineStageName)}>{PIPELINE_STAGES.map((name) => <option value={name} key={name}>{STAGE_LABELS[name]}</option>)}</select></label><button onClick={() => void rerun()} disabled={busy}>{busy ? "提交中…" : "创建修正任务"}</button><label>目标事件<select value={eventId ?? ""} onChange={(event) => setEventId(Number(event.target.value))}>{events.map((event) => <option value={event.id} key={event.id}>{event.event_type} · {event.title}</option>)}</select></label><label>归属角色<select value={role} onChange={(event) => setRole(event.target.value)}><option value="primary">primary</option><option value="component">component</option><option value="cross_ref">cross_ref</option></select></label><button onClick={() => void assign()} disabled={busy || !eventId}>手动归属事件</button>{message && <span>{message}</span>}</div>;
}
