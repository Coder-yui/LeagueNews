...
| `get_event` | Read one current public event's detail projection. Events without a current published mention are not returned. |
| `get_daily_report` | Read a published report for an ISO date with compact published message summaries. Use `get_news_item` for full content or evidence. |
| `get_latest_daily_report` | Read the newest published report with compact message summaries, or return a clear not-found tool error when none exists. |
...
- `MCP_SERVICE_TOKEN` is the secret value and must come from the environment when MCP is enabled in production;
...
- an empty token keeps local development convenient;
- production Compose sets `MCP_PRODUCTION=true`: when `MCP_ENABLED=true`, startup requires both
  a non-empty `MCP_SERVICE_TOKEN` and `MCP_ALLOWED_HOSTS`; when `MCP_ENABLED=false`, neither is
  required and the `/mcp` endpoint is not mounted.

The SDK's DNS-rebinding protection is enabled for configured hosts. When enabled, production must
set `MCP_ALLOWED_HOSTS` to the public host (for example `news.example.com`); optional browser
origins are configured with `MCP_ALLOWED_ORIGINS`.
...
To disable MCP in production, set `MCP_ENABLED=false` and leave `MCP_SERVICE_TOKEN` and
`MCP_ALLOWED_HOSTS` unset. The API still starts normally, but `/mcp` is not mounted. The
production environment example contains the same configuration names. Do not commit a real
token.
