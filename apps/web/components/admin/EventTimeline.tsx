...
import { pointScore, relativeTime, score } from "./admin-utils";
...
export function EventTimeline({ event, onExpire }: { event: EventDetail; onExpire?: () => void }) {
  ...
  return (
    <section className="admin-event-timeline">
      ...
      <footer><span>当前状态 <b>{event.lifecycle_status}</b></span><span>事件重要性 <b>{pointScore(event.importance_score)} / 100</b></span><span>事件可信度 <b>{score(event.credibility_score)}</b></span><span>支持/反对 <b>{event.supporting_source_count}/{event.contradicting_source_count}</b></span></footer>
    </section>
  );
}
